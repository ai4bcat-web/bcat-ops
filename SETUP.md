...
3. Under **Project Settings → Script properties**, set `FACTORING_WEBHOOK_URL` to the deployed **FactoringIntakeFunctionUrl** CloudFormation output. Set `FACTORING_WEBHOOK_SECRET` to the existing Amplify `INTAKE_WEBHOOK_SECRET`, or reuse the project's existing `WEBHOOK_SECRET` constant. Never put secret values in logs or a committed script.
