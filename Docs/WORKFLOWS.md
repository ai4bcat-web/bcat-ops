...
- A manual dispute on `/disputes` takes the same documents as the portal: the driver comes from the active roster dropdown, the 7-day period is a Sunday-week dropdown (prefilled from the shipment date, legacy hand-typed values stay selectable), a trip confirmation is required, and up to 5 supporting images or PDFs can be attached (10 MB each, no SVG). Staff uploads live under `dispute-staff-proofs/{disputeId}/` — their own prefix, so staff can replace or remove them while the driver's `dispute-proofs/` files stay untouchable. Editing a dispute keeps every stored file unless staff remove a staff-owned one, and a failed save cleans up the objects it just uploaded.
