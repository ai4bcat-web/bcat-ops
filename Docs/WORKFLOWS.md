...
- A fixed expense is shared through the pay split by default (an after-expenses driver bears only their pay % of it). For a charge the driver owes in full — lease share, lease mileage, IFTA — pick **Driver pays in full, after the split** on the line; it then comes off the check at 100% after the % model and shows under Debits (after net). An optional company share on the same line stays off the statement and is booked as a company cost in Finances. Weekly mileage can be a mileage-calculation line with that option (Change → new miles → effective date of the week), or a one-week **Debit** on the settlement (reason Lease mileage / IFTA fuel tax) — same math either way.
