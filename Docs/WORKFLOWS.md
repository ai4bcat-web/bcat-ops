...
- Setting a dispute to Paid offers "Add to settlement week": a Sunday week (next week back through ten) plus the driver's pay account, prefilled by matching the typed portal name against the active roster. Saving writes a `DriverPayCredit` (`kind CREDIT`, `reasonCode DISPUTE`, amount = recovered, label = trip · shipment date) so the recovery shows as a DISPUTE credit at 100% on that week's Amazon settlement, and stores `settlementCreditId`/`settlementPeriodStart`/`settlementDriverId` on the dispute row.
- That credit id is what keeps a recovery from being paid twice: re-saving moves or re-prices the same credit, choosing "Don't add" or leaving Paid deletes it. A credit someone deleted on the settlement page is written fresh; any other write failure surfaces instead of creating a second credit. A recovery with no amount keyed is refused. The dispute row shows the settlement week it was paid out on.
