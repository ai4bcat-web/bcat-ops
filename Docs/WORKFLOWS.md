...
- The same page shows a shared status board: driver name, trip number, pay period, shipment date, status, and the amount recovered — for every dispute. The recovered amount is the ONLY figure the public endpoint returns (and only once a row is Paid); requested/paid amounts, descriptions, uploads and staff notes never leave the authenticated app. Every driver on the board sees every other driver's recovered amount.
