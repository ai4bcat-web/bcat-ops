...
## Factoring queue
- Finance → **Factoring Queue** (`/factoring`, page grant `factoring`) tracks one row per PRO from the subject `Invoice for PRO #12345`; forwards, mixed case, whitespace, a missing `#`, trailing punctuation and alphanumeric PROs (`945143JJ`) are accepted. Leading zeroes are preserved and letters are upper-cased.
