...
- Drivers file disputes at `/amazon-disputes` (public, no login). The form requires the trip confirmation email (any image type - screenshot, photo, HEIC - or a PDF) and accepts up to 5 supporting files (images or PDFs); files go to `dispute-proofs/{submissionId}/` via a presigned PUT, then the `dispute-portal-api` Lambda verifies each object and writes one `AmazonDispute` row (`source DRIVER_PORTAL`, `status PENDING`). Retrying a submission reuses its id, so a retry never creates a second row.
