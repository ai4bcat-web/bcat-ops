        ...
        {selectedRow && (
          <StatementCard
            ...
            onWaiveDeduction={async (label, amount) => {
              if (!window.confirm(`Waive ${label} (${money(amount)}) for this period only?\n\nAdds an offsetting refund line; every other period keeps the charge.`)) return
              try { await pay.addDeduction({ driverId: selectedRow.driver.id, periodStart, label: `Waived — ${label}`, amount: -amount, date: null }); toast.success(`${label} waived for this period`) }
...
function StatementCard({ row, onPull, onAddTrip, onImport, onAddDeduction, onAddCredit, onAddDebit, onEditCredit, onRemoveCredit, onSettings, onEditTrip, onRemoveTrip, onPushTrip, onRemoveDeduction, onWaiveDeduction, onExport, onPdf, onEmail, onUpdateTrip }: {
  row: BoxTruckPayRow
  ...
  onRemoveDeduction: (id: string) => void; onWaiveDeduction: (label: string, amount: number) => void; onExport: () => void
