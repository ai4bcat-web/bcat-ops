...
export function ApplicationForm({ driverId, initial, onSaveDraft, onSubmit, onExit }: Props) {
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-6">
      ...
      <p className="mb-4 text-sm" style={{ color: 'var(--ds-t3)' }}>Step {step + 1} of {STEPS.length}</p>

      {step === 0 && (
        <div className="space-y-3">
          <Labeled label="Full legal name"><input style={inputStyle} value={data.legalName ?? ''} onChange={(e) => set('legalName', e.target.value)} /></Labeled>
          <Labeled label="Date of birth"><input type="date" style={inputStyle} value={data.dob ?? ''} onChange={(e) => set('dob', e.target.value)} /></Labeled>
          <Labeled label="SSN — last 4 digits only"><input inputMode="numeric" maxLength={4} style={inputStyle} value={data.ssnLast4 ?? ''} onChange={(e) => set('ssnLast4', e.target.value.replace(/\D/g, ''))} /></Labeled>
          <Labeled label="Phone"><input type="tel" style={inputStyle} value={data.phone ?? ''} onChange={(e) => set('phone', e.target.value)} /></Labeled>
          <Labeled label="Current address"><input style={inputStyle} value={data.currentAddress ?? ''} onChange={(e) => set('currentAddress', e.target.value)} /></Labeled>
        </div>
      )}

      {step === 1 && (
        <div className="space-y-3">
          <p className="text-sm" style={{ color: 'var(--ds-t3)' }}>List where you've lived over the past 3 years (49 CFR 391.21).</p>
          {addresses.map((a, i) => (
            <div key={i} className="space-y-2" style={subCard}>
              <input style={inputStyle} placeholder="Street" value={a.street ?? ''} onChange={(e) => updateItem('addressHistory', i, { street: e.target.value })} />
              <div className="grid grid-cols-3 gap-2">
                <input style={inputStyle} placeholder="City" value={a.city ?? ''} onChange={(e) => updateItem('addressHistory', i, { city: e.target.value })} />
                <input style={inputStyle} placeholder="State" value={a.state ?? ''} onChange={(e) => updateItem('addressHistory', i, { state: e.target.value })} />
                <input style={inputStyle} placeholder="ZIP" value={a.zip ?? ''} onChange={(e) => updateItem('addressHistory', i, { zip: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Labeled label="From"><input type="date" style={inputStyle} value={a.fromDate ?? ''} onChange={(e) => updateItem('addressHistory', i, { fromDate: e.target.value })} /></Labeled>
                <Labeled label="To (blank = present)"><input type="date" style={inputStyle} value={a.toDate ?? ''} onChange={(e) => updateItem('addressHistory', i, { toDate: e.target.value })} /></Labeled>
              </div>
              <button onClick={() => removeItem('addressHistory', i)} style={removeBtn}><Trash2 size={13} /> Remove</button>
            </div>
          ))}
          <button onClick={() => addItem('addressHistory', { street: '', city: '', state: '', zip: '', fromDate: '', toDate: '' })} style={linkBtn}><Plus size={14} /> Add residence</button>
      ...
      {step === 2 && (
        <div className="space-y-3">
          <Labeled label="CDL number"><input style={inputStyle} value={data.cdlNumber ?? ''} onChange={(e) => set('cdlNumber', e.target.value)} /></Labeled>
          <div className="grid grid-cols-2 gap-2">
            <Labeled label="State"><input style={inputStyle} value={data.cdlState ?? ''} onChange={(e) => set('cdlState', e.target.value)} /></Labeled>
            <Labeled label="Class"><input style={inputStyle} value={data.cdlClass ?? ''} onChange={(e) => set('cdlClass', e.target.value)} /></Labeled>
          </div>
          <Labeled label="Expiration"><input type="date" style={inputStyle} value={data.cdlExpiration ?? ''} onChange={(e) => set('cdlExpiration', e.target.value)} /></Labeled>
          <Labeled label="Endorsements (comma-separated)"><input style={inputStyle} value={(data.endorsements ?? []).join(', ')} onChange={(e) => set('endorsements', e.target.value.split(',').map((s) => s.trim()).filter(Boolean))} /></Labeled>
          ...
          {data.cdlIssuedAfterFeb2022 && (
            <Labeled label="ELDT training provider"><input style={inputStyle} value={data.eldtProviderName ?? ''} onChange={(e) => set('eldtProviderName', e.target.value)} /></Labeled>
          )}
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-3">
          <p className="text-sm" style={{ color: 'var(--ds-t3)' }}>Account for the last {requiredYears} years. We'll flag any gap over 30 days for you to explain.</p>
          {employment.map((e, i) => (
            <div key={i} className="space-y-2" style={subCard}>
              <input style={inputStyle} placeholder="Employer name" value={e.employerName ?? ''} onChange={(ev) => updateItem('employmentHistory', i, { employerName: ev.target.value })} />
              <input style={inputStyle} placeholder="Address" value={e.address ?? ''} onChange={(ev) => updateItem('employmentHistory', i, { address: ev.target.value })} />
              <div className="grid grid-cols-2 gap-2">
                <input style={inputStyle} placeholder="Phone" value={e.phone ?? ''} onChange={(ev) => updateItem('employmentHistory', i, { phone: ev.target.value })} />
                <input style={inputStyle} placeholder="Position" value={e.position ?? ''} onChange={(ev) => updateItem('employmentHistory', i, { position: ev.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Labeled label="From"><input type="date" style={inputStyle} value={e.fromDate ?? ''} onChange={(ev) => updateItem('employmentHistory', i, { fromDate: ev.target.value })} /></Labeled>
              ...
              <input style={inputStyle} placeholder="Reason for leaving" value={e.reasonForLeaving ?? ''} onChange={(ev) => updateItem('employmentHistory', i, { reasonForLeaving: ev.target.value })} />
              ...
              {gaps[i] && (
                <div className="rounded-md p-2" style={{ background: 'var(--ds-amber-bg)' }}>
                  <div className="text-xs font-medium" style={{ color: 'var(--ds-amber)' }}>Gap of {gaps[i].days} days before this job — please explain</div>
          ...
          <button onClick={() => addItem('employmentHistory', { employerName: '', address: '', phone: '', fromDate: '', toDate: '', position: '', reasonForLeaving: '', subjectToFMCSR: false, safetySensitive: false })} style={linkBtn}><Plus size={14} /> Add employer</button>
        ...
        <div className="space-y-4">
          <div>
            ...
            {(data.accidents ?? []).map((a, i) => (
              <div key={i} className="mb-2 space-y-2" style={subCard}>
                <input type="date" style={inputStyle} value={a.date ?? ''} onChange={(e) => updateItem('accidents', i, { date: e.target.value })} />
                <input style={inputStyle} placeholder="Nature of accident" value={a.nature ?? ''} onChange={(e) => updateItem('accidents', i, { nature: e.target.value })} />
                <button onClick={() => removeItem('accidents', i)} style={removeBtn}>Remove</button>
              </div>
            ))}
          ...
          <div>
            ...
            {(data.violations ?? []).map((v, i) => (
              <div key={i} className="mb-2 space-y-2" style={subCard}>
                <input type="date" style={inputStyle} value={v.date ?? ''} onChange={(e) => updateItem('violations', i, { date: e.target.value })} />
                <input style={inputStyle} placeholder="Offense" value={v.offense ?? ''} onChange={(e) => updateItem('violations', i, { offense: e.target.value })} />
                <button onClick={() => removeItem('violations', i)} style={removeBtn}>Remove</button>
              </div>
            ))}
      ...
      {step === 5 && (
        <div className="space-y-4">
          ...
          <Labeled label="Type your full legal name to sign"><input style={inputStyle} value={data.signatureName ?? ''} onChange={(e) => set('signatureName', e.target.value)} /></Labeled>
