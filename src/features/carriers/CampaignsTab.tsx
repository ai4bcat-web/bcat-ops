  ...
  useEffect(() => {
    ...
    carrierBlast('listAccounts')
      .then((res) => {
        ...
        setAccounts((res.accounts as InstantlyAccount[]) ?? [])
      })
...
function CampaignComposer({
  ...
  const [perMailboxLimit, setPerMailboxLimit] = useState<number>(30)
  ...
  const minSelectedLimit = selectedAccounts.length > 0
    ? Math.min(...selectedAccounts.map((email) => okAccounts.find((a) => a.email === email)?.dailyLimit ?? 0))
    : 0
  const effectivePerMailbox = selectedAccounts.length > 0 ? Math.min(perMailboxLimit, minSelectedLimit || Infinity) : 0
  const totalDailyCap = effectivePerMailbox * selectedAccounts.length
  const days = activeCount && totalDailyCap ? Math.ceil(activeCount / totalDailyCap) : null
  ...
  return (
    <div style={{ background: 'var(--ds-surface)', border: '1px solid var(--ds-border)', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, alignItems: 'end' }}>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-t2)', display: 'block', marginBottom: 6 }}>
            Per-mailbox daily limit (max {minSelectedLimit || '—'})
          </label>
          <Input
            type="number"
            min={1}
            max={minSelectedLimit || undefined}
            value={perMailboxLimit}
            onChange={(e) => setPerMailboxLimit(e.target.value === '' ? 30 : Number(e.target.value))}
            placeholder="30"
          />
        </div>
        <div data-testid="campaign-estimate" style={{ fontSize: 13, color: 'var(--ds-t2)', background: 'var(--ds-bg)', borderRadius: 8, padding: '10px 12px' }}>
          ...
          {totalDailyCap === 0
            ...
            : `${totalDailyCap} emails/day across ${selectedAccounts.length} mailbox${selectedAccounts.length === 1 ? '' : 'es'}`}
          {days != null && ` · ~${days} day${days === 1 ? '' : 's'} to reach ${activeCount} active contacts`}
        </div>
      </div>
