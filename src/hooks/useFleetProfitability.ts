...
export function useFleetProfitability(range: DateRange, group: FleetGroup): FleetProfitabilityHookResult {
  ...
  const data = useMemo<FleetProfitabilityResult | null>(() => {
    if (members.length === 0) {
      return { range, trucks: [], rollup: { revenue: 0, miles: 0, fuel: 0, insurance: 0, loan: 0, otherExpenses: 0, driverCost: 0, net: 0, revenuePerMile: null, fuelPerMile: null } }
    }
  ...
  }, [range, members, loads, fuel.transactions, exp.records, exp.recurring, exp.allocations, exp.expenseTypes, mileage.rows, pay.payPeriods, drivers])
