...
| `/appts` | Appointments board — EVERY load, ONE ROW PER SHIPMENT, with separate PU Status / Del Status chips (green Booked; red NEED or Pending — FCFS/range stops count as booked since they need no call) and PU + delivery locations/drivers side by side, sectioned by pickup day with booked/open counts (each day exportable as CSV), past-dated loads hidden by default, NEED-first ordering, edit the scheduled PU/delivery times in place or book from the calendar (`src/lib/apptQueue.ts`; the sidebar badge counts only outstanding shipments). Each row expands into a booking-history timeline (set/moved/unbooked, by whom, when) derived from the load's existing `AuditLog` entries — no second event store (`src/lib/apptHistory.ts`). Appointment status is saved exactly as picked: typing a time into a NEED stop no longer auto-graduates it to Exact (`apptTypeAfterEdit`) |
