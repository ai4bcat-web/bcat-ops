...
| `/driver-pay` | Amazon driver weekly (7-day) trip-based pay + statement PDFs/email — now with the same settlement credits/debits as box-truck (added/subtracted at 100% after the % model) and per-week waivers of fixed charges |
| `/driver-pay-box-trucks` | Box-truck (Ivan Cartage) biweekly shipment-based pay — settlement credits (detention/layover/bonus, paid at 100%), debits (cash advance/damage/escrow, off the check at 100% after the net), one-off deductions (negative = refund), and pushing a shipment forward to the next pay period |
...
**Driver pay:** `DriverPayPeriod` (biweekly gross pay, Paychex seam) · `AmazonTrip` (weekly trip-based Amazon pay lines) · `AmazonPayMaster` (archive of uploaded master CSVs, raw file in S3) · `BoxTruckTrip` (biweekly box-truck shipment lines) · `DriverPaySetting` (per-driver pay model: %, expense timing, fuel card, fixed deductions; `rateHistory` pins past rate windows `[{from, until, payPercent, expensesBeforePercent}]` — pay weeks starting in `[from, until)` use that pinned model via `effectivePayRate()` in `src/lib/driverPay.ts`, so changing a driver's current rate never rewrites already-paid weeks; each fixed expense likewise carries optional `from`/`until` period-start bounds applied per week by `effectiveFixedExpenses()` — end a charge by setting `until`, delete only for mistakes, and a per-week waive button writes an offsetting negative one-off instead) · `DriverPayDeduction` (per-week one-off charges; negative amount = refund, shown green) · `DriverPayCredit` (settlement money that didn't come from shipment gross profit, on both Amazon weekly and box-truck settlements; `kind` is `'CREDIT'` — default for null/pre-existing rows, extra pay like detention/layover/bonus/reimbursement added at 100% — or `'DEBIT'` — cash advance/damage/escrow/overpayment subtracted at 100% AFTER the % model runs, so check = net + credits − debits; the pay % is NOT applied to either, `reasonCode` is a plain string so the list can grow without a redeploy — see `src/lib/payCredits.ts`)
...
- **appt-report** — daily 3 PM America/Chicago digest to the global Slack channel listing every appointment in the next 5 business days NOT yet confirmed (Batory ladder statuses, plus non-Batory loads still missing their ratecon); weekdays only, scans the Load table directly; two UTC cron rules (20:00 & 21:00) fire it and the handler posts only when it's actually 15:00 in Chicago, so DST never shifts the report
